...
def main():
    ...
    while True:
        ...
        if choice == '0':
            ...
            print(f"{'序号':<6} {'日期':<12} {'类型':<6} {'分类':<6} {'金额':<8} 描述")
            print("-" * 60)
            for i, t in enumerate(sorted(transactions, key=lambda x: x.date, reverse=True)):
                print(
                    f"{i:<6} {t.date.isoformat():<12} {t.type:<6} {t.category:<6} {t.amount:<8} {t.description}")
            ...
            if sub_choice == '1':
                ...
                if not transactions:
                    print("没有收入记录")
                    continue
                print(f"{'序号':<6} {'日期':<12} {'分类':<6} {'金额':<8} 描述")
                print("-" * 50)
                for i, t in enumerate(sorted(transactions, key=lambda x: x.date, reverse=True)):
                    print(
                        f"{i:<6} {t.date.isoformat():<12} {t.category:<6} {t.amount:<8} {t.description}")
                ...
                if not count:
                    print("没有收入记录可删除")
                    continue
                confirm = input(f"确定要删除所有{len(count)}条收入记录吗? (y/n): ")
                if confirm.lower() == 'y':
                ...
                if not count:
                    print("没有支出记录可删除")
                    continue
                confirm = input(f"确定要删除所有{len(count)}条支出记录吗? (y/n): ")
                if confirm.lower() == 'y':
