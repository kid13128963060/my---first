...
if __name__ == "__main__":
    ...
    if args.create:
        ...
        if args.new_entity_name:
            ...
            # 确保文件包含扩展名（无论是否已有点号）
            if is_file := os.path.isfile(os.path.join(args.old_folder, full_name)) if os.path.exists(os.path.join(args.old_folder, full_name)) else True:
                full_name = f"{full_name}{args.ext}" if not full_name.endswith(args.ext) else full_name
        else:
        ...
        if args.new_entity_name:
            full_name = args.new_entity_name.replace('-', '_')
            # 确保文件包含扩展名（无论是否已有点号）
            full_name = f"{full_name}{args.ext}" if is_file and not full_name.endswith(args.ext) else full_name
        else:
            full_name = generate_filename(
                args.prefix, args.lang, args.version,
